...
export function mergeScopes(oldAllowed: string[], newAllowed: string[], oldDenied: string[], newDenied: string[]) {
  const newDeniedScopes = _.uniq(oldDenied.concat(newDenied)).filter(
    (denied) => !newAllowed.includes(denied)
  )
  const newAllowedScopes = _.uniq(oldAllowed.concat(newAllowed)).filter(

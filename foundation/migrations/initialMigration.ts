...
export async function up(knex: Knex) {
	const tableExists = await knex.schema.hasTable('UserAccounts')

	if (tableExists) {
		//this is an existing installation.  We need to update tables
		if (await knex.schema.hasColumn('Sessions', 'fingerprint')) {
			await knex.schema.table('Sessions', (table) => {
				table.dropColumn('fingerprint')
			})
		}
		if (await knex.schema.hasColumn('EmailAddresses', 'verificationSecret')) {
			await knex.schema.table('EmailAddresses', (table) => {
				table.dropColumn('verificationSecret')
			})
		}
		if (await knex.schema.hasColumn('PhoneNumbers', 'verificationSecret')) {
			await knex.schema.table('PhoneNumbers', (table) => {
				table.dropColumn('verificationSecret')
			})
		}
		if (await knex.schema.hasColumn('Settings', 'createdAt')) {
...
export async function down(knex: Knex) {
	await knex.schema
		...
		.dropTable('KeyValueCache')
		.dropTable('Clients')
		.dropTable('UserClients')
